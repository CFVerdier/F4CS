...
class Solution:
    ...
    ---------------------
        specification (spec): specification from the RWS class
    ...
    def __init__(self, specification, template={}):
        ...
        self.k_sym = template.get(
            'controller', sp.zeros(len(specification.input), 1))
        self.V_sym = template.get('certificate', 0)

        self.dV_sym = sp.Matrix([self.V_sym]).jacobian(specification.var)
        self.dtV_sym = sp.Matrix(self.dV_sym).dot(
            specification.system).subs(zip(specification.input, self.k_sym))
        self.dV_sym = None
        self.dVfun = None
        ...
        self.conditions_sym = specification.create_conditions(self)
        # Create symbolic fitness function for this individual.
        self.create_fitness(specification)
    ...
    def create_fitness(self, specification):
        ...
        for condition in self.conditions_sym:
            ...
            f = f.replace(sp.StrictGreaterThan, lambda x,
                          y: x >= y + specification.c)
            f = f.replace(sp.StrictLessThan, lambda x,
                          y: x <= y - specification.c)
            f = f.replace(sp.LessThan, lambda x, y: -x >= -y)
            f = f.replace(sp.GreaterThan, lambda x,
                          y: x-y-specification.epsilon)
            # Replace booleans
            ...
            function = sp.lambdify(
                self.p + specification.extended_var, f,
                modules=[{'amax': custom_amax, 'amin': custom_amin}, "numpy"])
            result = result + (function,)
        self.fitness_function = result
